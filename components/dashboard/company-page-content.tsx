  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card className="border-border bg-card">
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Button variant={uploadType === "pitch_deck" ? "default" : "outline"} size="sm" onClick={() => setUploadType("pitch_deck")}>
              Pitch Deck
            </Button>
          ...
          <label className={cn("block border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors", "border-border hover:border-primary/50")}>
            <input type="file" accept=".pdf,.txt,.md,.html,.csv" multiple disabled={uploading} onChange={handleFileChange} className="hidden" />
            {uploading ? (
          ...
          {assets.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="space-y-1">
                {assets.map((a) => (
                  <div key={a.id} className="flex items-center gap-2 px-3 py-2 rounded-md bg-muted/50 text-[13px]">
      ...
      <Card className="border-border bg-card">
        <CardHeader>
          <button onClick={() => setShowContextPreview(!showContextPreview)} className="flex items-center justify-between w-full text-left">
            <div>
              <CardTitle className="text-[15px] flex items-center gap-2">
